...
class Ball:
    def __init__(self,canvas,paddle, d_paddle,color,announceterm,saveterm,winval=10,loseval=-1):
        self.canvas = canvas
        self.paddle = paddle
        self.d_paddle = d_paddle
    ...
    def action(self):
        ...
        if r < self.epsilon:
            ...
            d_direction = self.d_randomChoice()
        else:
            direction = self.greedyChoice()
            d_direction = self.greedyChoice()
        x = PADDLE_MOVE[direction]
        d_x = PADDLE_MOVE[d_direction]
        # 머신러닝을 위한 한 사이클 내 이동 데이터 저장
        self.cycle_data.append(self.keystate(direction))
        self.d_cycle_data.append(self.d_keystate(d_direction))
        # 이동/그리기
        self.paddle.move(x)
        self.paddle.draw()
        self.d_paddle.move(d_x)
        self.d_paddle.draw()
    ...
    def d_randomChoice(self):
        rand = random.choice([1,0])
        key = self.d_keystate(rand)
        if key not in self.values:
            self.d_add(key)
        return rand
    ...
    def d_greedyChoice(self):
        val_left = self.d_keystate(0)
        val_right = self.d_keystate(1)

        if self.d_lookup(val_left) > self.d_lookup(val_right):
            return 0
        elif self.d_lookup(val_left) < self.d_lookup(val_right):
            return 1
        else:
    ...
    def d_lookup(self, key):
        if key not in self.d_values:
            # print(key)
            self.d_add(key)
        return self.d_values[key]
    ...
    def d_hit_paddle(self, pos):  # 패들에 공이 튀기게 하는 함수
        paddle_pos = self.canvas.coords(self.d_paddle.id)
        print(paddle_pos)
        # 공의 x좌표가 패들의 너비 안에 있는지 / 공 바닥이 패들 윗면에 닿아 있는지
        if pos[2] >= paddle_pos[0] and pos[0] <= paddle_pos[2] \
                and pos[1] == D_PADDLE_HEIGHT:
            return True
        return False
    ...
    def d_is_paddle_hit(self):
        return self.d_hit_paddle(self.canvas.coords(self.id))
    ...
    def d_keystate(self, movement):
        d_paddle_pos = self.canvas.coords(self.d_paddle.id)
        ball_pos = self.canvas.coords(self.id)
        # paddle 위치(좌측 x좌표), 공의 좌상단 x/y좌표, 공의 좌우/상하 속도(방향), paddle을 좌/우 중 어느 쪽으로 움직이는지
        return (d_paddle_pos[0], (ball_pos[0], ball_pos[1]), (self.x, self.y), movement)
    ...
    def d_cyclestate(self):
        pos = self.canvas.coords(self.id)
        if pos[3] == D_PADDLE_HEIGHT:
            if self.y == 3:
                return START
            elif self.y == -3:
                return END
...
if __name__ == '__main__':
    ...
    d_paddle = Paddle(canvas,D_PADDLE_HEIGHT,'red')

    # announceterm : 현재 count 출력 term, saveterm : csv에 저장하는 term
    ball = Ball(canvas, paddle,d_paddle, 'orange', announceterm=500, saveterm=500)
    start = False
