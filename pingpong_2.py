...
class Ball:
    ...
    def __init__(self, canvas, down_paddle, up_paddle, color):

        self.canvas = canvas
        self.down_paddle = down_paddle
        self.up_paddle = up_paddle
    ...
    def d_hit_paddle(self, pos):  # 아래패들에 공이 튀기게 하는 함수

        paddle_pos = self.canvas.coords(self.down_paddle.id)
        print(paddle_pos)
    ...
    def u_hit_paddle(self, pos):  # 윗 패들에 공이 튀기게 하는 함수

        paddle_pos = self.canvas.coords(self.up_paddle.id)

        if pos[2] >= paddle_pos[0] and pos[0] <= paddle_pos[2]:  # 공이 패들에 올라오기 직전 좌표
            if pos[3] >= paddle_pos[1] and pos[3] <= paddle_pos[3]:  # 공이 패들에 닿았을때 좌표
                return True
        return False
    ...
    def draw(self):
        ...
        if self.d_hit_paddle(pos) == True:  # 패들 판에 부딪히면 위로 튕겨올라가게

            self.y = -3  # 공을 위로 올린다.

        if self.u_hit_paddle(pos) == True:  # 패들 판에 부딪히면 아래로 튕겨올라가게
...
class down_Paddle:
    ...
    def __init__(self, canvas, color):
        ...
        self.canvas.move(self.id, 200, 400)  # 패들 사각형을 200,300 에 위치
    ...
    def draw(self):
        ...
        d_pos = self.canvas.coords(self.id)
        #print(pos)

        if d_pos[0] <= 0 and self.x < 0:  # 패들의 위치가 왼쪽 끝이고, 이동하려는 방향이 왼쪽이면 함수 종료(이동 안 함)

            return

        elif d_pos[2] >= self.canvas_width and self.x > 0: #패들의 위치가 오른쪽 끝이고,이동하려는 방향이 오른쪽이면 종료
...
class up_Paddle(down_Paddle):
    def __init__(self, canvas, color):
        super(up_Paddle, self).__init__(canvas, color)
        self.canvas=canvas
        self.canvas.move(self.id, 200, 80)
...
up_paddle = up_Paddle(canvas, 'blue')

down_paddle = down_Paddle(canvas,'blue')

ball = Ball(canvas, down_paddle,up_paddle, 'red')
